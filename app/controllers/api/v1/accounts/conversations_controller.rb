class Api::V1::Accounts::ConversationsController < Api::V1::Accounts::BaseController
  ...
  before_action :set_conversation, only: [:show, :update, :toggle_status, :skip]
  ...
  def skip
    unless @conversation && @conversation.account_id == current_account.id
      render json: { error: 'Conversation not found or unauthorized' }, status: :not_found
      return
    end

    conversation_skip = ConversationSkips::CreateService.new(
      conversation: @conversation,
      agent: current_user,
      reason: params[:reason]
    ).perform

    render json: { message: 'Conversation skipped successfully', conversation_skip_id: conversation_skip.id }, status: :created
  rescue StandardError => e
    render json: { error: e.message }, status: :unprocessable_entity
  end

  def next_open
    conversation = Conversations::NextOpenConversationService.new(
    ...
    if conversation
      ...
      render json: { message: 'No more open conversations available.' }, status: :not_found
    end
